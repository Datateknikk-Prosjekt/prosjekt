...
const uint16_t sensorThresholdDark = 600; //grensen for om "AboveLineDark" blir returnert true.
...
/*bool aboveLineDark(uint8_t ) //funksjon som blir returnert true vist sensoren ser mye mørkt.
{
...
void followline(){
  ...
  if (abs(position - lastposition) > 500) { //if setning som blir kjørt vist forskjellen mellom lastposition og position
    beforeMillis = millis();
 ...
 /*
  ...
  if (aboveLineDark == HIGH){
  display.clear();
  display.print("D"); 
  }
  if (aboveLineDark == LOW){
  display.clear();
  display.print("L"); 
  }
*/
